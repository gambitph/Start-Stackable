...
Human developers: start at [`docs/prd/start-stackable.md`](./docs/prd/start-stackable.md) (current state, how we develop, then product rules).
...
**Default**:
The site you get on theme activate with Stackable off: designed blog, header, footer, tokens, and templates.
_Avoid_: Default Site Kit (it is not a kit package), demo import, starter site

**Shell**:
Theme-owned header, footer, blog, archives, 404, search, page canvases, and `theme.json`.
_Avoid_: layout (too vague), page builder, Design Library, chrome
...
A kit-owned `theme.json` styles object the plugin writes as user Global Styles so the header and footer match the kit without a theme release.
...
**Shell pattern**:
A theme pattern that reconstructs Default (header, footer, blog atoms, template guts) or the one DIY Homepage starter page.
...
_Avoid_: inventing kit-only CSS in the plugin to fake the header or footer, chrome
