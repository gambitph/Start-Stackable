...
| `e2e/tests/header-flags.spec.ts` | Sticky and transparent body flags normalize onto the header wrapper; the header overlays a full-width hero, becomes solid after scroll with contrast-safe text, exposes a non-zero height token, and keeps a full-viewport, closable mobile navigation overlay above the hero. Long desktop Navigation stays on one line by moving only trailing links into an accessible More disclosure, then restores every link on wide desktop and in core's mobile overlay. |
