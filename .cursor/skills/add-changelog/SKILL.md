...
name: add-changelog
description: >-
  Adds or updates a WordPress changelog entry in readme.txt from the
  project's Release Roadmap for a confirmed theme or plugin version. Use when
  the user asks to add a changelog, write changelog entries, update readme.txt
...
Add or update a `== Changelog ==` entry in `readme.txt` for a confirmed version, using issues and PRs from the project's Release Roadmap.
...
- Main product repo(s)
- Companion plugin / premium repos (if any)
...
Find the product version from the WordPress header `Version:` field:

1. If this is a theme, use the `Version:` header in root `style.css`.
2. Else prefer `plugin.php` in the project root.
3. Else `{plugin-slug}.php` / `{ProjectName}.php` (main plugin bootstrap file).
4. Optionally note `Stable tag:` in `readme.txt` for context; the **header Version** is the default proposal.

### 2. Confirm version with user (required)

**Stop and ask** before collecting roadmap items or editing files.

Tell the user the proposed version (e.g. `Found Version: 1.0.0 in style.css`). Ask them to:
